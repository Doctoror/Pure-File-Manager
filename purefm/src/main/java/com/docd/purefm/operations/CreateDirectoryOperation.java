...
/**
 * Creates a single directory. Result is null on success, otherwise the result is a error message to
 * display to user
 *
 * @author Doctoror
 */
final class CreateDirectoryOperation extends Operation<GenericFile, CharSequence> {

    @NonNull
    private final Context mContext;

    CreateDirectoryOperation(@NonNull final Context context) {
        mContext = context;
    }
    ...
    protected CharSequence doInBackground(@NonNull final GenericFile... genericFiles) {
        ...
        try {
            ...
            if (!target.mkdir()) {
                return mContext.getText(R.string.could_not_create_dir);
            } else {
